...
pub async fn authorize_handler(
    ...
    if params.response_type != "code" {
        ...
        return create_error_redirect(&params.redirect_uri, "unsupported_response_type", "Response type must be 'code'", params.state.as_deref());
    }
    
    // Validate PKCE parameters
    if params.code_challenge.is_empty() {
        error!("Missing required PKCE code_challenge parameter");
        return create_error_redirect(&params.redirect_uri, "invalid_request", "Missing code_challenge parameter", params.state.as_deref());
    }
    ...
    if code_challenge_method != "S256" && code_challenge_method != "plain" {
        ...
        return create_error_redirect(&params.redirect_uri, "invalid_request", "Invalid code_challenge_method", params.state.as_deref());
    }
...
pub async fn oauth_token_handler(
    ...
    if params.grant_type != "authorization_code" {
        ...
        return Err(create_oauth2_error(StatusCode::BAD_REQUEST, "unsupported_grant_type", "Grant type must be 'authorization_code'"));
    }
    
    // Retrieve and consume authorization code
    let auth_code = state.consume_auth_code(&params.code)
        .ok_or_else(|| {
            error!(code = %params.code, "Invalid or expired authorization code");
            create_oauth2_error(StatusCode::BAD_REQUEST, "invalid_grant", "Invalid or expired authorization code")
        })?;
    ...
    if auth_code.client_id != params.client_id {
        ...
        return Err(create_oauth2_error(StatusCode::BAD_REQUEST, "invalid_grant", "Client ID mismatch"));
    }
    ...
    if auth_code.redirect_uri != params.redirect_uri {
        ...
        return Err(create_oauth2_error(StatusCode::BAD_REQUEST, "invalid_grant", "Redirect URI mismatch"));
    }
    
    // Verify PKCE code_verifier
    if !verify_pkce_challenge(&params.code_verifier, &auth_code.code_challenge, &auth_code.code_challenge_method) {
        error!(
            code_challenge = %auth_code.code_challenge,
            code_challenge_method = %auth_code.code_challenge_method,
            "PKCE verification failed"
        );
        return Err(create_oauth2_error(StatusCode::BAD_REQUEST, "invalid_grant", "PKCE verification failed"));
    }
    ...
    let jwt_token = generate_test_token(
        ...
        .map_err(|e| {
            ...
            create_oauth2_error(StatusCode::INTERNAL_SERVER_ERROR, "server_error", "Failed to generate access token")
        })?;
...
pub async fn oauth2_metadata_handler() -> Result<Json<Value>, StatusCode> {
    ...
    debug!("OAuth2 metadata: {}", serde_json::to_string_pretty(&metadata).unwrap_or_else(|_| "Failed to serialize".to_string()));
